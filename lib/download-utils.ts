...
export async function downloadFile(url: string, options: DownloadOptions = {}): Promise<void> {
  ...
  try {
    ...
    if (url.includes('s3.') || url.includes('amazonaws.com') || url.includes('backblazeb2.com')) {
      try {
        await downloadViaDirectLink(url, filename, onComplete);
        return;
      } catch (directError) {
        console.log('Direct link download failed, trying fetch approach:', directError);
    ...
    const downloadError = error instanceof Error ? error : new Error('Network error during download');
    if (onError) {
 ...
 */
async function downloadViaDirectLink(url: string, filename?: string, onComplete?: (filename: string) => void): Promise<void> {
 ...
 * @param options - Download options including progress callbacks
 */
export async function downloadFileViaApi(downloadUrl: string, options: DownloadOptions = {}): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      xhr.onload = () => {
        if (xhr.status === 200) {
          ...
          if (!downloadFilename) {
            downloadFilename = extractFilenameFromUrl(downloadUrl) || 'download';
          }
          ...
          try {
            ...
            if (xhr.status === 401) {
              ...
              errorMessage = 'Invalid download request. Please check the URL and try again.';
      ...
      xhr.onerror = () => {
        const error = new Error('Network error during download. Please check your internet connection.');
        if (onError) {
          onError(error);
        }
        reject(error);
      ...
      xhr.ontimeout = () => {
        const error = new Error('Download timeout. The file may be too large or the connection is slow.');
        if (onError) {
          onError(error);
        }
        reject(error);
